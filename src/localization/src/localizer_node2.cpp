...
public:
  ...
  Eigen::Matrix4f align_map(const pcl::PointCloud<pcl::PointXYZI>::Ptr scan_points){
    ...
    pass1.setInputCloud(scan_points);
    pass1.setFilterFieldName("x");
    pass1.setFilterLimits(-35, 35);
    pass1.filter(*filtered_scan_ptr);
    // set y filter 
    pcl::PassThrough<pcl::PointXYZI> pass2;
    pass2.setInputCloud(scan_points);
    pass2.setFilterFieldName("y");
    pass2.setFilterLimits(10, 25);
    ...
    if(!initialied){
      ...
      for  (yaw = 4; yaw < 5; yaw += 0.4) {
        ...
        if (score < min_score) {
            ...
            std::cout << "Update best pose" << std::endl;
  ...
  // trans << gps_point.x, gps_point.y, gps_point.z;
  // init_guess.block<3,1>(0,3) = trans;
